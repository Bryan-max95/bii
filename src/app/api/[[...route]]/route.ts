...
async function seedUserData(userId: number) {
  ...
  try {
    ...
    if (parseInt(existingDevices.rows[0].count) === 0) {
      ...
      console.log(`✅ Seeded ${devices.length} devices for user ${userId}`);
    }
    ...
    if (parseInt(existingIncidents.rows[0].count) === 0) {
      ...
      console.log(`✅ Seeded ${incidents.length} incidents for user ${userId}`);
    }
  } catch (err) {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (pathname === '/api/auth/register') {
      ...
      // Validar correo corporativo para empresas
      if (userType === 'company') {
...
export async function PUT(request: NextRequest) {
  const pathname = request.nextUrl.pathname;
  console.log('📥 PUT request:', pathname);
...
export async function DELETE(request: NextRequest) {
  const pathname = request.nextUrl.pathname;
  console.log('📥 DELETE request:', pathname);
